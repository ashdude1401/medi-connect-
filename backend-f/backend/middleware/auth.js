...
const isAuthenticatedOrg = catchAsyncErrors(async(req,res,next) => {
    const { token } = req.cookies;
    if(!token){
        return next(new ErrorHandler("Please Login to access this resource", 401));
    }
...
const authorizeRoles = (...roles) => {
    return (req, res, next) => {
        ...
        //roles-admin
        //req.user.role-user    
        //it checks if req.user.role is admin then does the array roles contain admin? if not then this condition runs else next() runs
